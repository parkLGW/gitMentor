...
5) searchTerms 给 2-6 个**英文**关键词，用来在英文文件名/路径里做检索：把问题里的中文概念翻译成代码里常见的英文命名（如“鉴权/登录”→ auth、login；“工具”→ tools、registry；“记忆/会话”→ memory、session）。即使问题是英文，也要提炼最能命中文件名的核心词。
6) 不要输出 markdown 代码块。`
  }
...
5) searchTerms: give 2-6 English keywords for matching against file names/paths, translating the question's concepts into the naming actually used in code (e.g. "auth"/"login", "tools"/"registry", "memory"/"session"). Provide them even for English questions, distilling the terms most likely to appear in a file name.
